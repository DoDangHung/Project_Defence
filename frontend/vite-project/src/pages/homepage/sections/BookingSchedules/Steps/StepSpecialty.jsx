...
import { Filter, X, ChevronDown, ChevronUp, Search, Stethoscope, Heart } from 'lucide-react';
...
function StepSpecialty() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const categoryRes = await axios.get(`${API_URL}/service-categories?withSpecialties=true&isActive=true&_t=${Date.now()}`, {
  ...
  const specialistCategory = categories.find(c => c.slug === 'kham-chuyen-khoa');

  const generalSpecialties = allSpecialties.filter(s => s.categoryId === generalCategory?.id);
  const specialistSpecialties = allSpecialties.filter(s => s.categoryId === specialistCategory?.id);
  const otherSpecialties = allSpecialties.filter(
    s => s.categoryId !== generalCategory?.id && s.categoryId !== specialistCategory?.id
  );

  const displayedSpecialties = viewMode === 'general'
    ? generalSpecialties
    : viewMode === 'specialist'
  ...
  return (
    <>
      <section className="max-w-7xl mx-auto px-4 py-16">
        <h2 className="text-center text-xl font-semibold mb-8">CHỌN HÌNH THỨC ĐẶT KHÁM</h2>
        ...
        {/* Khám Tổng Quát Section */}
        {(viewMode === 'general' || viewMode === 'all') && generalSpecialties.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">
                <Stethoscope className="w-5 h-5 text-blue-600" />
              </div>
              <h3 className="text-lg font-bold text-gray-800">Khám tổng quát</h3>
              <span className="text-sm text-gray-500">({generalSpecialties.length} chuyên khoa)</span>
            </div>
              ...
              {generalSpecialties.map((spec) => (
                ...
                >
                  ...
                  <span className={`mt-2 text-sm font-medium transition-colors ${
                    selectedSlug === spec.slug ? 'text-blue-600' : 'text-gray-800 group-hover:text-blue-600'
                  }`}>
                    {spec.name}
                  </span>
                </button>
        ...
        {/* Divider */}
        {(viewMode === 'all') && generalSpecialties.length > 0 && specialistSpecialties.length > 0 && (
        ...
        {/* Khám Chuyên Khoa Section */}
        {(viewMode === 'specialist' || viewMode === 'all') && specialistSpecialties.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center">
                <Heart className="w-5 h-5 text-orange-600" />
              </div>
              <h3 className="text-lg font-bold text-gray-800">Khám chuyên khoa</h3>
              <span className="text-sm text-gray-500">({specialistSpecialties.length} chuyên khoa)</span>
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-6">
              {specialistSpecialties.map((spec) => (
                ...
                >
                  ...
                  <span className={`mt-2 text-sm font-medium transition-colors ${
                    selectedSlug === spec.slug ? 'text-orange-600' : 'text-gray-800 group-hover:text-orange-600'
                  }`}>
                    {spec.name}
                  </span>
                </button>
        ...
        {(viewMode === 'all') && otherSpecialties.length > 0 && (
          <>
            <div className="flex items-center gap-4 my-8">
              <div className="flex-1 h-px bg-gray-200"></div>
              <span className="text-gray-400 text-sm">Dịch vụ khác</span>
              <div className="flex-1 h-px bg-gray-200"></div>
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-6">
              {otherSpecialties.map((spec) => (
                ...
                >
                  ...
                  <span className={`mt-2 text-sm font-medium transition-colors ${
                    selectedSlug === spec.slug ? 'text-purple-600' : 'text-gray-800 group-hover:text-purple-600'
                  }`}>
                    {spec.name}
                  </span>
                </button>
